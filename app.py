...
class App(object):
    ...
    def init(self):
        ...
        self.fov = FOV
        self.screen = Screen(self.size, self.fov, (self.size[0]/2, self.size[1]/2))
        self.cube = Cube(A)
        self.a = self.cube.a
    ...
    def __loop__(self):
        ...
        if self.fovspeed > 0:
            to_fovmove = self.fovspeed*clock_factor
            if self.fovdir == 0: # increasing
                self.fov += self.fovspeed
                if self.fov >= 160:
                    self.fov = 160
                    self.fovdir = 1
            if self.fovdir == 1: # decreasing
                self.fov -= self.fovspeed
                if self.fov <= 10:
                    self.fov = 10
                    self.fovdir = 0
            self.screen.set_fov(self.fov)
    ...
    def __render__(self):
        ...
        coords[1] += self.a/2
        coords[2] += self.a/2
        cube_points = self.screen.transform_coords(self.cube.vert+self.top_left)
        ...
        fov_text = self.small_font.render(f"FOV: {self.fov}   a: {self.a}", 1, (0,0,0))
